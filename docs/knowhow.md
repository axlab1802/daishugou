...
## 8. Vercel Serverlessでの状態保持
**課題**: ルーム状態をメモリに置くとサーバーレス環境で消える。
**対応**: Vercel Redis にルームを保存し、`room-lock` で同時更新を抑制。状態取得時にTTL延長。 

## 9. ポーリングの負荷と遅延
**課題**: 常時800ms固定ポーリングだと混雑時に失敗が続く。
**対応**: 失敗時にポーリング間隔を段階的に延長し、成功時にリセットするバックオフを導入。

## 10. モバイルでの角ラベル視認性
**課題**: モバイルでカードの酒名が読みにくい。  
**対応**: `tools/card_layout.json` に角ラベル用の座標を追加し、`tools/generate_cards.py` で上下の数字横に酒名を同サイズで描画するようにした。

## 11. モバイル1画面レイアウト
**課題**: 場のカードがスクロールしないと見えない。  
**対応**: モバイル時は上部に相手情報と場を集約し、手札エリア右下に操作ボタンを重ねて配置するレイアウトに変更。
